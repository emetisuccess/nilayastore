	...
	if (f.name.value == '') {
		...
		jQuery.ajax({
			...
			complete: function (data) {
				...
				jQuery('#fruitkha-contact').find('input,textarea').attr({
					value: ''
				});
				jQuery('#fruitkha-contact').css({
					opacity: 1
				});
		...
		jQuery('#fruitkha-contact').animate({
			opacity: 0.3
		});
		jQuery('#fruitkha-contact').find('input,textarea,button').css('border', 'none').attr({
			'disabled': ''
		});
	}
